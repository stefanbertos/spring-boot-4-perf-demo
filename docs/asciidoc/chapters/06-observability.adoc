...
NOTE: `initialDelaySeconds` for liveness must be at least 60s to account for Spring Boot + ZGC startup time.
Services with more dependencies (ibm-mq-consumer) may need 90s.

...
Build images first, then start all services using the provided script:

[source,bash]
----
# Build all OCI images and start Docker Compose
...
* All application modules (perf-tester, ibm-mq-consumer, kafka-consumer, api-gateway, config-server)
* IBM MQ
* Kafka
* Redis
* Prometheus
...
[source,bash]
----
# Start a test (returns 202 with testRunId immediately)
curl -X POST "http://localhost:8080/api/perf/send?count=100&timeoutSeconds=30" \
  -H "Content-Type: text/plain" \
  -d "Hello, World!"

# Stream real-time progress (SSE)
curl -N "http://localhost:8080/api/perf/progress/<testRunId>"
----
